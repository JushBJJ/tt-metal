...
void bind_op_with_mem_config_and_dtype(
    ...
    if constexpr (mem_config_arg && dtype_arg) {
        ...
        docstring += fmt::format(
            ...
            "{0}", "Layout of tensor in TT Accelerator device memory banks", "MemoryConfig", "Default is {1} in {2}", "No")doc",
            mem_config_name,
            magic_enum::enum_name(operation::DEFAULT_OUTPUT_MEMORY_CONFIG.memory_layout),
            magic_enum::enum_name(operation::DEFAULT_OUTPUT_MEMORY_CONFIG.buffer_type));
        const std::string dtype_name = "output_dtype";
        docstring += fmt::format(
            R"doc(
            "{0}", "Output tensor data type", "DataType", "Default is None (Use input dtype)", "No")doc",
        ...
        "{0}", "Layout of tensor in TT Accelerator device memory banks", "MemoryConfig", "Default is {1} in {2}", "No")doc",
            ...
            magic_enum::enum_name(operation::DEFAULT_OUTPUT_MEMORY_CONFIG.buffer_type));
        if constexpr (kernel_config) {
            std::cout << "op name: " << op_name << " bind with kernel config" << std::endl;
            module.def(
                op_name.c_str(),
                f,
                std::forward<Extra>(extra)...,
            ...
            std::cout << "op name: " << op_name << " bind w/o kernel config" << std::endl;
            module.def(
                op_name.c_str(),
                f,
                std::forward<Extra>(extra)...,
        ...
        if constexpr (kernel_config) {
            module.def(
                ...
                py::arg(kernel_config_name.c_str()).noconvert() = std::nullopt,
                docstring.c_str());
            std::cout << "op name: " << op_name << " bind with kernel config" << " config name: " << kernel_config_name << std::endl;
        } else {
