...
std::vector<ttnn::Tensor> _atan2_bw(
    ...
    Tensor recip_mul = ttnn::multiply(
        grad,
        tt::tt_metal::unary_chain(hypot(input, other), {op1, op2}, output_mem_config),
        std::nullopt,
        output_mem_config);
        ...
        if (other_grad.has_value()) {
            ttnn::multiply(
                ...
                ttnn::operations::creation::full_like(
                    grad, alpha, grad.get_dtype(), grad.get_layout(), std::nullopt, output_mem_config),
                std::nullopt,
...
std::vector<ttnn::Tensor> _addalpha_bw_inter(
    ...
    auto result =
        _addalpha_bw(0, grad, input, other, alpha, output_mem_config, {true, true}, std::nullopt, std::nullopt);
...
std::vector<std::optional<Tensor>> _add_bw(
    ...
    std::optional<Tensor> other_grad) {
    return _addalpha_bw(
        queue_id, grad, input, other, 1.0f, output_mem_config, are_required_outputs, input_grad, other_grad);
}
...
std::vector<std::optional<Tensor>> _add_bw_overload(
    ...
    uint8_t default_queue_id = 0;
    return _addalpha_bw(
        default_queue_id, grad, input, other, 1.0f, output_mem_config, are_required_outputs, input_grad, other_grad);
}
...
std::vector<ttnn::Tensor> _xlogy_bw(
    ...
    Tensor zero_tensor = ttnn::operations::creation::zeros_like(
        other, other.get_dtype(), other.get_layout(), std::nullopt, output_mem_config);
    grad1_result = where(
        ttnn::logical_and(
    ...
    grad2_result = where(
        ...
        ttnn::multiply(
            sign(grad, output_mem_config), std::numeric_limits<float>::infinity(), std::nullopt, output_mem_config),
        grad2_result,
        output_mem_config);
    grad2_result =
...
std::vector<ttnn::Tensor> _hypot_bw(
    ...
    Tensor grad_a = ttnn::multiply(
        grad, ttnn::multiply(input, result_recip, std::nullopt, output_mem_config), std::nullopt, output_mem_config);
    grad_tensor.emplace_back(grad_a);
    Tensor grad_b = ttnn::multiply(
        grad, ttnn::multiply(other, result_recip, std::nullopt, output_mem_config), std::nullopt, output_mem_config);
    grad_tensor.emplace_back(grad_b);
    return grad_tensor;
}
...
std::vector<ttnn::Tensor> _ldexp_bw(
    ...
    Tensor result = ttnn::multiply(
        input, ttnn::multiply(tpow_o, M_LN2, std::nullopt, output_mem_config), std::nullopt, output_mem_config);
...
std::vector<ttnn::Tensor> _logaddexp2_bw(
    ...
    Tensor oppow = ttnn::add(
        rpow(ttnn::subtract(other, input_a, std::nullopt, output_mem_config), 2, output_mem_config),
        1,
        std::nullopt,
        output_mem_config);
    ...
    grad_tensor.emplace_back(grad_a);
    oppow = ttnn::add(
        rpow(ttnn::subtract(input_a, other, std::nullopt, output_mem_config), 2, output_mem_config),
        1,
        std::nullopt,
        output_mem_config);
...
std::vector<Tensor> _binary_le_bw(
    const Tensor& grad, const Tensor& input, const Tensor& other, const MemoryConfig& output_mem_config) {
    std::vector<Tensor> grad_tensor;
...
std::function<std::vector<ttnn::Tensor>(const Tensor&, const Tensor&, const Tensor&, const MemoryConfig&)>
get_function_type1(BinaryBackwardOpType OpType) {
    switch (OpType) {
        case BinaryBackwardOpType::ATAN2_BW: return _atan2_bw;
        case BinaryBackwardOpType::SUB_BW: return _sub_bw;
        case BinaryBackwardOpType::XLOGY_BW: return _xlogy_bw;
        case BinaryBackwardOpType::HYPOT_BW: return _hypot_bw;
        case BinaryBackwardOpType::LDEXP_BW: return _ldexp_bw;
        case BinaryBackwardOpType::LOGADDEXP_BW: return _logaddexp_bw;
        case BinaryBackwardOpType::LOGADDEXP2_BW: return _logaddexp2_bw;
        case BinaryBackwardOpType::SQUARED_DIFFERENCE_BW: return _squared_difference_bw;
        case BinaryBackwardOpType::ADD_BW: return _add_bw_inter;
        case BinaryBackwardOpType::BINARY_EQ_BW: return _binary_eq_bw_inter;
        case BinaryBackwardOpType::BINARY_ASSIGN_BW: return _binary_assign_bw;
        case BinaryBackwardOpType::BINARY_LE_BW: return _binary_le_bw;
        case BinaryBackwardOpType::RSUB_BW: return _rsub_bw;
        default: TT_ASSERT(false && "Undefined op type"); return 0;
    }
}

std::function<std::vector<ttnn::Tensor>(const Tensor&, const Tensor&, const Tensor&, float, const MemoryConfig&)>
get_function_type1_w_float(BinaryBackwardOpType OpType) {
    switch (OpType) {
        case BinaryBackwardOpType::SUBALPHA_BW: return _subalpha_bw;
        case BinaryBackwardOpType::ADDALPHA_BW: return _addalpha_bw_inter;
        case BinaryBackwardOpType::CONCAT_BW: return _concat_bw;
        default: TT_ASSERT(false && "Undefined op type"); return 0;
    }
}

std::function<std::vector<ttnn::Tensor>(const Tensor&, const Tensor&, const Tensor&, string, const MemoryConfig&)>
get_function_type1_w_string(BinaryBackwardOpType OpType) {
    switch (OpType) {
        case BinaryBackwardOpType::BIAS_GELU_BW: return _bias_gelu_bw;
        default: TT_ASSERT(false && "Undefined op type"); return 0;
    }
}

std::function<std::vector<std::optional<ttnn::Tensor>>(
    uint8_t,
    const Tensor&,
    const Tensor&,
    const Tensor&,
    ...
    std::optional<Tensor>,
    std::optional<Tensor>)>
get_function_type2(BinaryBackwardOpType OpType) {
    switch (OpType) {
        case BinaryBackwardOpType::ADDALPHA_BW: return _addalpha_bw;
        default: TT_ASSERT(false && "Undefined op type"); return 0;
    }
}
...
std::function<std::vector<std::optional<ttnn::Tensor>>(
    ...
    std::optional<Tensor>,
    std::optional<Tensor>)>
get_function_type2_wo_qid(BinaryBackwardOpType OpType) {
    switch (OpType) {
        case BinaryBackwardOpType::ADDALPHA_BW: return _addalpha_bw_overload;
        default: TT_ASSERT(false && "Undefined op type"); return 0;
    }
}
...
std::function<std::vector<std::optional<ttnn::Tensor>>(
    ...
    std::optional<Tensor>,
    std::optional<Tensor>)>
get_function_type3(BinaryBackwardOpType OpType) {
    switch (OpType) {
        case BinaryBackwardOpType::ADD_BW: return _add_bw;
        case BinaryBackwardOpType::BINARY_EQ_BW: return _binary_eq_bw;
        default: TT_ASSERT(false && "Undefined op type"); return 0;
    }
}
...
std::function<std::vector<std::optional<ttnn::Tensor>>(
    ...
    std::optional<Tensor>,
    std::optional<Tensor>)>
get_function_type3_wo_qid(BinaryBackwardOpType OpType) {
    switch (OpType) {
        case BinaryBackwardOpType::ADD_BW: return _add_bw_overload;
        case BinaryBackwardOpType::BINARY_EQ_BW: return _binary_eq_bw_overload;
        default: TT_ASSERT(false && "Undefined op type"); return 0;
    }
}
}  // namespace utils
