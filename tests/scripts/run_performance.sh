...
run_perf_models_llm_javelin() {
    ...
    if [ "$tt_arch" == "wormhole_b0" ]; then
        env WH_ARCH_YAML=wormhole_b0_80_arch_eth_dispatch.yaml env pytest -n auto models/demos/wormhole/mamba/tests -m $test_marker --timeout=360
    fi
...
run_device_perf_models() {
    ...
    if [ "$tt_arch" == "wormhole_b0" ]; then
        env WH_ARCH_YAML=wormhole_b0_80_arch_eth_dispatch.yaml env pytest models/demos/wormhole/mamba/tests -m $test_marker
