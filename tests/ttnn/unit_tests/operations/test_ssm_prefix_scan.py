...
def run_ssm_prefix_scan(L: int, E: int, N: int, num_cores: int, dtype, device):
    ...
    actual = ttnn.experimental.ssm.prefix_scan(a, bx, h_prev, memory_config=memory_config, dtype=dtype)
...
def run_chunked_ssm_prefix_scan(L: int, E: int, N: int, chunk_size: int, num_cores: int, dtype, device):
    ...
    for idx in range(len(a_chunks)):
        ...
        h_chunk = ttnn.experimental.ssm.prefix_scan(a_chunk, bx_chunk, h_prev, memory_config=memory_config, dtype=dtype)
