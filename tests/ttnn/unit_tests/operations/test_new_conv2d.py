...
def run_conv(
    ...
    # torch_output_tensor is in row major layout and NHWC shape
    # NHWC to NCHW
    print(batch_size, out_height, out_width, output_channels)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        # (1, 48, 32, 124, 124, 3, 3, 1, 1, 3, 3, 2, 2, 1, "relu"), #  Conv2: dilation>1
        # (1, 56, 48, 136, 136, 3, 3, 1, 1, 0, 0, 4, 4, 1, "relu"),  # Conv3: dilation>1
        # (1, 64, 56, 112, 112, 3, 3, 1, 1, 0, 0, 8, 8, 1, "relu"),  # Conv4: dilation>1
        ...
        # (1, 1, 256, 95, 95, 1, 1, 1, 1, 0, 0, 1, 1, 1, ""),  # Conv7: Passed
    ),
)
@pytest.mark.parametrize(
    "config_override",
...
@pytest.mark.parametrize("math_fidelity", [ttnn.MathFidelity.LoFi])
def test_klassify_new_conv_128x128(
    device,
    math_fidelity,
    activations_dtype,
    weights_dtype,
...
@pytest.mark.parametrize(
    "batch_size, output_channels, input_channels, input_height, input_width, filter_height, filter_width, stride_h, stride_w, pad_h, pad_w, dilation_h, dilation_w, groups, activation",
    (
        (1, 32, 256, 96, 96, 1, 1, 1, 1, 0, 0, 1, 1, 1, ""),  # Conv7
        (1, 32, 3, 128, 128, 5, 5, 1, 1, 0, 0, 1, 1, 1, "relu"),  # Conv1
    ),
)
@pytest.mark.parametrize(
    "config_override",
...
@pytest.mark.parametrize("math_fidelity", [ttnn.MathFidelity.LoFi])
def test_klassify_new_conv_128x128_unpad(
    device,
    math_fidelity,
    activations_dtype,
    weights_dtype,
