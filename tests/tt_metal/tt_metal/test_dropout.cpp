...
bool test_dropout(float probability, uint32_t seed,  float const_bias, std::vector<bfloat16>& res_vec) {
    bool pass = true;
    uint32_t int_probability = probability * (float)INT_MAX;
...
int main(int argc, char **argv) {
    ...
    srand(0);
    float magic_number = 9.0;
    bool pass = true;
    int num_tests = 10;
    for(int i = 0; i <= num_tests; i++) {
	float probability = (float)i/(float)num_tests;
	uint32_t seed = rand();
	std::vector<bfloat16> res_0, res_1;
	pass &= test_dropout(probability, seed, magic_number, res_0);
	pass &= test_dropout(probability, seed, magic_number, res_1);
	bool repeatable = std::equal(res_0.begin(), res_0.end(), res_1.begin());
	if(!repeatable) {
           tt::log_error(tt::LogTest, "Same parameters gave different results probability={}, seed={}", probability, seed);
	} else {
