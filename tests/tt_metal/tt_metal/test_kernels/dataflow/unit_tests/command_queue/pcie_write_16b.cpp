...
void kernel_main() {
    ...
    DPRINT << "pcie core noc encoding " << HEX() << pcie_core_noc_encoding << DEC() << ENDL();
    ...
    for (uint32_t i = 0; i < num_16b_writes; i++) {
        ...
        DPRINT << "hi register gets: " << HEX() << (uint32_t(dst_noc_addr >> 36)) << DEC()
               << " lo register gets: " << HEX() << (uint32_t)(dst_noc_addr) << DEC()
               << " pcie dst addr: " << HEX() << pcie_dst_address << DEC() << ENDL();
